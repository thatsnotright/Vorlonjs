...
var VORLON;
(function (VORLON) {
    var DOMTimelineClient = (function (_super) {
        __extends(DOMTimelineClient, _super);
        function DOMTimelineClient() {
            _super.call(this, "domtimeline"); // Name
            this._ready = true; // No need to wait
            //console.log('Started');
        }
        //Return unique id for your plugin
        DOMTimelineClient.prototype.getID = function () {
            return "DOMTIMELINE";
        };
        DOMTimelineClient.prototype.refresh = function () {
        ...
        DOMTimelineClient.prototype.startClientSide = function () {
            var domData = null;
            ...
            domTimelineOptions.considerLoggingRecords = function (c, entries, s) {
                if (!domData)
                    return console.warn("domData not yet initalized; entries ignored: ", entries);
                ...
                function ensureDomDataIsUpToDate(e) {
            ...
            domHistory.generateDashboardData = function (knownData) {
                var data = {};
                // basic data
                ...
                // lostFuture
                data.lostFuture = (domHistory.lostFuture.map(getDashboardDataForEntry));
            ...
            function generateDashboardDataForEntry(e) {
                var targetDescription = descriptionOf(e.target);
                if (e.addedNodes.length > 0) {
                    var nodeDescription = ((e.addedNodes.length == 1)
                    ...
                    return {
                        ...
                        details: {
                            ...
                            "Claim": e.claim,
                            "Stack": "`" + e.stack.split('\n').filter(function (l) { return l.indexOf("vorlon.max") == -1; }).join('\n') + "`"
                        },
                        rawData: {
                            type: "childList",
                    ...
                    var nodeDescription = ((e.removedNodes.length == 1)
            ...
            function descriptionOf(e) {
                ...
                if (e.nodeName[0] != "#") {
                    return "`attribute " + e.nodeName + "`";
                }
                if (e.nodeValue.length < 15) {
                    return "`" + e.nodeValue + "`";
                }
                return "`" + e.nodeValue.substr(0, 15) + "…`";
            }
            function descriptionOfNodeList(elements) {
                if (elements.length == 0) {
                    return "`empty node list`";
                }
                if (elements.length == 1) {
                    return "`1 node: " + descriptionOf(elements[0]).replace(/^`|`$/g, '') + "`";
                }
        ...
        DOMTimelineClient.prototype.onRealtimeMessageReceivedFromDashboardSide = function (receivedObject) {
            var result;
            try {
        ...
        return DOMTimelineClient;
    }(VORLON.ClientPlugin));
    VORLON.DOMTimelineClient = DOMTimelineClient;
    //Register the plugin with vorlon core
    VORLON.Core.RegisterClientPlugin(new DOMTimelineClient());
})(VORLON || (VORLON = {}));
