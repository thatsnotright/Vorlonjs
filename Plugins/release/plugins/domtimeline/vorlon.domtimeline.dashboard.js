...
var VORLON;
(function (VORLON) {
    var DOMTimelineDashboard = (function (_super) {
        __extends(DOMTimelineDashboard, _super);
        //Do any setup you need, call super to configure
        //the plugin with html and css for the dashboard
        function DOMTimelineDashboard() {
            //     name   ,  html for dash   css for dash
            _super.call(this, "domtimeline", "control.html", "control.css");
            this._carbonCopyHandlers = [];
            this._ready = true;
        ...
        DOMTimelineDashboard.prototype.getID = function () {
            return "DOMTIMELINE";
        };
        DOMTimelineDashboard.prototype.startDashboardSide = function (div) {
            var _this = this;
            if (div === void 0) { div = null; }
            this._insertHtmlContentAsync(div, function (applicationDiv) {
                //this._outputDiv = <HTMLElement>filledDiv.querySelector('#output');
                //this._toastDiv = <HTMLElement>filledDiv.querySelector('#toast');
                var me = _this;
                var dashboard = initDashboard(function (s) { me.sendMessageToClient(s); });
                // Handle toolbar buttons
                var clientCommands = applicationDiv.querySelectorAll("[data-client-command]");
                for (var i = clientCommands.length; i--;) {
                    var clientCommand = clientCommands[i];
                    clientCommand.onclick = function (event) {
                        me.sendMessageToClient(this.getAttribute("data-client-command"), function (e) { return me.logMessage(e); });
                    };
                }
                // Handle the settings pane
                var settingsPane = applicationDiv.querySelector('#settings');
                var settingsPaneOpenButton = applicationDiv.querySelector("#open-settings-button");
                var settingsPaneCloseButton = settingsPane.querySelector("#close-settings-button");
                settingsPaneOpenButton.addEventListener('click', function (e) {
                    applicationDiv.setAttribute('is-settings-pane-open', 'true');
                    settingsPane.focus();
                });
                settingsPaneCloseButton.addEventListener('click', function (e) {
                    applicationDiv.setAttribute('is-settings-pane-open', 'false');
                    settingsPaneOpenButton.focus();
                });
                settingsPane.addEventListener('keyup', function (e) {
                    if (e.keyCode == 27) {
                        settingsPaneCloseButton.click();
                    }
                });
                // settings>dombreakpoints
                var breakpointsCheckbox = settingsPane.querySelector('#are-breakpoints-enabled');
                var breakpointsCodeTextbox = settingsPane.querySelector('#breakpoints-code');
                var saveBreakpointsCodeForm = settingsPane.querySelector('#breakpoints-code-form');
                saveBreakpointsCodeForm.addEventListener('submit', function (e) {
                    var code = (breakpointsCheckbox.checked
                ...
                breakpointsCheckbox.addEventListener('change', function (e) {
                    var code = (breakpointsCheckbox.checked
                ...
                var updateTimer = setInterval(function () {
                    me.sendMessageToClient("domHistory.generateDashboardData(" + ("{history:" + alreadyKnownEvents.length + ",lostFuture:0,domData:" + domData.data.length + "}") + ")", function (e) {
                        // refresh metadata
                        ...
                        if (e.message.isRecordingEnded && !isIframeAlreadyCreated) {
                            ...
                            Helpers.implementDomHistoryInWindow({ clientUrl: clientUrl, events: alreadyKnownEvents, domData: domData }, popup);
                            popup.eval("domHistory.seek(" + lastPastEventsCount + ")");
                            me._carbonCopyHandlers.push(function (message) { return popup.eval(message); });
                        }
                    });
                }, 500);
            });
        ...
        DOMTimelineDashboard.prototype.logMessage = function (receivedObject) {
        ...
        DOMTimelineDashboard.prototype.sendMessageToClient = function (message, callback) {
            if (callback === void 0) { callback = undefined; }
            // send the message to the real client
            var messageId = this._messageId++;
            if (callback)
                this._messageHandlers[messageId] = callback;
            this.sendToClient({ message: message, messageId: messageId });
        ...
        DOMTimelineDashboard.prototype.onRealtimeMessageReceivedFromClientSide = function (receivedObject) {
        ...
        return DOMTimelineDashboard;
    }(VORLON.DashboardPlugin));
    VORLON.DOMTimelineDashboard = DOMTimelineDashboard;
    var Helpers = (function () {
        function Helpers() {
        }
        Helpers.implementDomHistoryInWindow = function (_a, popup) {
            var clientUrl = _a.clientUrl, events = _a.events, domData = _a.domData;
            var pDoc = popup.document;
            var doctypeText = getDoctypeText();
            pDoc.open();
            pDoc.write(doctypeText + "<html></html>");
            pDoc.close();
            var oRE = pDoc.documentElement;
            var oHE = oRE.querySelector("head") || createHeadFor(oRE);
            var oBE = oHE.querySelector("base") || createBaseFor(oHE);
            ...
            var nRE = getNewRootElement();
            var nHE = nRE.querySelector("head") || createHeadFor(nRE);
            var nBE = nHE.querySelector("base") || createBaseFor(nHE);
            ...
            function getNewRootElement() {
                var docStart = popupDomData.p2oMap.get("0:0");
                if (!("tagName" in docStart && docStart.tagName.toLowerCase() != 'parsererror')) {
                    docStart = popupDomData.p2oMap.get("1:0");
                }
                return docStart;
            }
            ...
            function preparePopupWithShim() {
                ...
                popup['domHistory'] = {
                    generateDashboardData: function (knownData) {
                        return {
                    ...
                    seek: function (newPastEventsCount) {
                ...
                function convertToMutationRecord(d) {
                    ...
                    if (e.target)
                        e.target = popupDomData.getObjectFor(e.target);
                    if (e.nextSibling)
                        e.nextSibling = popupDomData.getObjectFor(e.nextSibling);
                    if (e.addedNodes)
                        e.addedNodes = popupDomData.getObjectListFor(e.addedNodes);
                    if (e.removedNodes)
                        e.removedNodes = popupDomData.getObjectListFor(e.removedNodes);
                    return e;
                }
                // 
                ...
                function undoMutationRecord(change) {
                    switch (change.type) {
                        ...
                        case "attributes":
                            ...
                            if (change.attributeName == 'value')
                                change.target.value = change.oldValue || '';
                            return;
                        //
                        case "characterData":
                ...
                function redoMutationRecord(change) {
                    switch (change.type) {
                        ...
                        case "attributes":
                            ...
                            if (change.attributeName == 'value')
                                change.target.value = change.newValue || '';
                            return;
                        //
                        case "characterData":
    ...
    VORLON.Helpers = Helpers;
    //Register the plugin with vorlon core
    VORLON.Core.RegisterDashboardPlugin(new DOMTimelineDashboard());
})(VORLON || (VORLON = {}));
function initDashboard(executeScriptOnClient) {
    var SCALE = 0.5;
    var START_TIME = 0;
    ...
    var setChanges = function (changes) {
        var html = '';
        var scroller = document.querySelector(".plugin-domtimeline");
        var scrollTop = scroller.scrollTop;
        if (scrollTop >= scroller.scrollHeight - scroller.offsetHeight - 1) {
            scrollTop = 9e10;
        }
        var times = {};
        ...
        function fixCode(v0) {
            if (v0[0] == '`' && v0.split('').reduce(function (i, c) { return (i + (c == '`' ? 1 : 0)); }, 0) % 2 == 1) {
                return v0 + '`';
            }
            else {
                return v0;
    ...
    var setTimeline = function (timeline, message) {
    ...
    var countByType = function (type, timeline) {
        ...
        for (var i = 0, len = timeline.length; i < len; i++) {
            if (timeline[i].type == type) {
                var isWithinSelection = (timeline[i].timestamp >= selectionBehavior.selectionStartTimestamp
    ...
    function enableActionButtonsForChangeEntries() {
        ...
        function onmousedown(e) {
            ...
            var li = target;
            while (li && li != this && li.tagName != 'LI') {
                li = li.parentElement;
            }
            ;
            if ($(li).hasClass('acc')) {
            ...
            var change = alreadyKnownEvents[li.dataset['id'] | 0];
    ...
    function enablePlayBehavior() {
        document.querySelector(".plugin-domtimeline #playStopButton").addEventListener('click', function (e) {
        ...
        return {
            ...
            playSelection: function () {
                ...
                playBehavior.isEnabled = true;
                document.querySelector(".plugin-domtimeline #dom-recorder").setAttribute('is-recording-playing', 'true');
    ...
    function enableSaveFileBehavior() {
        appDiv.querySelector("#saveAsFileButton").addEventListener('click', function (e) {
            ...
            var writeContent = function () {
                if (data.html && data.template && data.script) {
                    var content = data.template.replace(/\{\{([a-z]+)\}\}/gi, function (s, k) { return data[k]; });
            ...
            x.onload = function () {
                ...
                function onload() {
                    if (--imagecount == 0) {
                        data.html = html.replace(imageExtractor, function (x) { return images.shift(); });
                        writeContent();
                    }
                }
    ...
    function enableTimelineSeekBehavior() {
        function getSeekElement() {
            return document.querySelector("#timeline-seek");
        }
        var lastSeekTime = 0;
        ...
        timeline.addEventListener('mousedown', function (e) {
            ...
            function setSeekTimeBasedOnOffet(e) {
        ...
        function keepToolbarAndTimelineFixed() {
            var anchor = document.querySelector('#dom-recorder');
            var chromeFix = anchor.querySelector('#dom-recorder > #chrome-fix');
            var toolbar = anchor.querySelector('#dom-recorder > div[role="toolbar"]');
            var timeline = anchor.querySelector('#dom-recorder > nav');
        ...
        return {
            getSeekTime: function () { return getSeekTime(); },
            setSeekTime: function (v, syncBack) { setSeekTime(v, syncBack); }
        };
    }
    var selectionBehavior = enableTimelineHandleBehavior();
    function enableTimelineHandleBehavior() {
        var selectionZone = document.querySelector('#lev');
        var leftResizer = selectionZone.querySelector('.lev1');
        var rightResizer = selectionZone.querySelector('.lev2');
        selectionZone.addEventListener('mousedown', function (e) {
            e.stopPropagation();
            e.preventDefault();
            // disable in case of play animation
            if (playBehavior.isEnabled)
            ...
            var isValidSeekTime = (startSeekTime >= convertOffsetToTimestamp(startOffset - 1) &&
                startSeekTime <= convertOffsetToTimestamp(startOffset + deltaOffset + 1));
            if (!isValidSeekTime)
                startSeekTime = convertOffsetToTimestamp(startOffset + deltaOffset);
            window.addEventListener('mousemove', onmousemove, true);
            window.addEventListener('mouseup', onmouseup, true);
            function shiftSelectionBasedOnOffset(e) {
                var shift = (e.clientX - startClientX);
                if (Math.abs(shift) > 1)
                    shiftWasEverNonZero = true;
            ...
            function onmouseup(e) {
                ...
                if (shiftWasEverNonZero) {
                    ...
                    seekBehavior.setSeekTime(convertOffsetToTimestamp(e.clientX - selectionZone.parentElement.getBoundingClientRect().left), true);
                }
                window.removeEventListener('mousemove', onmousemove, true);
                window.removeEventListener('mouseup', onmouseup, true);
            }
        ...
        return {
            ...
            set: function (startTimestamp, endTimestamp) {
                this.isEnabled = true;
            ...
            syncFromDOM: function () {
                ...
                this.set(convertOffsetToTimestamp(offset1), convertOffsetToTimestamp(offset2));
            }
        };
    }
